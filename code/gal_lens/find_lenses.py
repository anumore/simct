...
from light2mass_new_Kcorrection import get_velocity_dispersion
...
##    -- Check the notebook 'example_find_potential_lenses.ipynb'                                       ##
##########################################################################################################
...
def find_potential_lenses(foreground_table, background_source, 
                          ...
                          parallel=False, save_source_file=True, chunk_offset=0):
    '''
    ...
    NOTE: the foreground catalog should have the above column names, else, change the column names as shown in
          the notebook 'example_find_potential_lenses.py'
    ...
    # Fix a seed
    myseed = 23524 + chunk_offset      #num*10+23424
    ...
    foreground_ymag,foreground_gmag, foreground_rmag, foreground_imag,foreground_zmag = foreground_table['ymag'],foreground_table['gmag'], foreground_table['rmag'], foreground_table['imag'],foreground_table['zmag']
    foreground_err_y,foreground_err_g,foreground_err_r,foreground_err_i,foreground_err_z = foreground_table['err_y'], foreground_table['err_g'], foreground_table['err_r'],foreground_table['err_i'],foreground_table['err_z']
    #find the axis ratio of the foreground
    foreground_axisratio = 1-foreground_ellipticity

    
    lsst_mags = np.array([foreground_gmag, foreground_rmag, foreground_imag, foreground_zmag, foreground_ymag]).T
    lsst_errs = np.array([foreground_err_g, foreground_err_r, foreground_err_i, foreground_err_z, foreground_err_y]).T
    ...
    foreground_velocity_dispersion = get_velocity_dispersion(deflector_type="elliptical", lsst_mags=lsst_mags, lsst_errs=lsst_errs, 
                                    redshift=foreground_redshift, cosmo=cosmo, bands=["g", "r", "i", "z", "y"],
    ...
    for ii in range(len(foreground_table)):
        ...
        if(foreground_gmag[ii]>0 and foreground_rmag[ii]>0 and foreground_redshift[ii] >kwargs_deflector.get('min_z') and foreground_redshift[ii]<kwargs_deflector.get('max_z')):
            ...
            #print('Printing foreground: ',foreground_imag[ii],foreground_redshift[ii],foreground_axisratio[ii])
            ...
            if(background_source_counts>0 and foreground_velocity_dispersion[ii]>0):
                for kk in range(background_source_counts):
                    #print(source_redshift_array[kk],foreground_redshift[ii])
                    ...
                    #print('------ background ',source_redshift_array[kk],source_magnitude_array[kk])
    ...
    if parallel:
        print('Chunk offset for this processor: ',chunk_offset)
        print(len(sorted_indices),' sources detected in this processor.')
        ...
        if save_source_file:
            ...
            #print(len(sorted_table),' sources found.')
        ...
        uniq_Einstein_radius = np.array(sorted_Einstein_radius)[ind_uniq]
        ...
        foreground_sorted_table['source_mag'] = np.array(sorted_source_mag)[ind_uniq]
        foreground_sorted_table['source_redshift'] = np.array(sorted_source_redshift)[ind_uniq]
        foreground_sorted_table['Einstein radius'] = uniq_Einstein_radius

        id_column = foreground_sorted_table['deflector Id']
        foreground_sorted_table.remove_column('deflector Id')
        foreground_sorted_table.add_column(id_column,index=0)

        # Save the final sorted lenses/ deflectors
        foreground_sorted_table.write('sorted_galaxies.csv',format='csv',overwrite=True)
        #print(foreground_sorted_table)
        return None
